...
struct _EPROCESS
{
    ...
    struct _MMSUPPORT Vm;                                                   //0x500
    struct _LIST_ENTRY MmProcessLinks;                                      //0x5f8
    ULONG ModifiedPageCount;                                                //0x608
    LONG ExitStatus;                                                        //0x60c
    struct _RTL_AVL_TREE VadRoot;                                           //0x610
    VOID* VadHint;                                                          //0x618
    ULONGLONG VadCount;                                                     //0x620
    volatile ULONGLONG VadPhysicalPages;                                    //0x628
    ULONGLONG VadPhysicalPagesLimit;                                        //0x630
    struct _ALPC_PROCESS_CONTEXT AlpcContext;                               //0x638
    struct _LIST_ENTRY TimerResolutionLink;                                 //0x658
    struct _PO_DIAG_STACK_RECORD* TimerResolutionStackRecord;               //0x668
    ULONG RequestedTimerResolution;                                         //0x670
    ULONG SmallestTimerResolution;                                          //0x674
    union _LARGE_INTEGER ExitTime;                                          //0x678
    struct _INVERTED_FUNCTION_TABLE* InvertedFunctionTable;                 //0x680
    struct _EX_PUSH_LOCK InvertedFunctionTableLock;                         //0x688
    ULONG ActiveThreadsHighWatermark;                                       //0x690
    ULONG LargePrivateVadCount;                                             //0x694
    struct _EX_PUSH_LOCK ThreadListLock;                                    //0x698
    VOID* WnfContext;                                                       //0x6a0
    ULONGLONG Spare0;                                                       //0x6a8
    UCHAR SignatureLevel;                                                   //0x6b0
    UCHAR SectionSignatureLevel;                                            //0x6b1
    struct _PS_PROTECTION Protection;                                       //0x6b2
    ...
    union
    {
        ...
        struct
        {
            ULONG Minimal:1;                                                //0x6b4
            ULONG ReplacingPageRoot:1;                                      //0x6b4
            ULONG DisableNonSystemFonts:1;                                  //0x6b4
            ULONG AuditNonSystemFontLoading:1;                              //0x6b4
            ULONG Crashed:1;                                                //0x6b4
            ULONG JobVadsAreTracked:1;                                      //0x6b4
            ULONG VadTrackingDisabled:1;                                    //0x6b4
            ULONG AuxiliaryProcess:1;                                       //0x6b4
            ULONG SubsystemProcess:1;                                       //0x6b4
            ULONG IndirectCpuSets:1;                                        //0x6b4
            ULONG InPrivate:1;                                              //0x6b4
            ULONG ProhibitRemoteImageMap:1;                                 //0x6b4
            ULONG ProhibitLowILImageMap:1;                                  //0x6b4
            ULONG SignatureMitigationOptIn:1;                               //0x6b4
        };
    };
    LONG DeviceAsid;                                                        //0x6b8
    VOID* SvmData;                                                          //0x6c0
    struct _EX_PUSH_LOCK SvmProcessLock;                                    //0x6c8
    ULONGLONG SvmLock;                                                      //0x6d0
    struct _LIST_ENTRY SvmProcessDeviceListHead;                            //0x6d8
    ULONGLONG LastFreezeInterruptTime;                                      //0x6e8
    struct _PROCESS_DISK_COUNTERS* DiskCounters;                            //0x6f0
    VOID* PicoContext;                                                      //0x6f8
    ...
    struct _PROCESS_ENERGY_VALUES* EnergyValues;                            //0x718
    VOID* VmContext;                                                        //0x720
    ...
    volatile ULONGLONG SharedCommitCharge;                                  //0x758
    struct _EX_PUSH_LOCK SharedCommitLock;                                  //0x760
    struct _LIST_ENTRY SharedCommitLinks;                                   //0x768
    union
    {
        struct
        {
            ULONGLONG AllowedCpuSets;                                       //0x778
            ULONGLONG DefaultCpuSets;                                       //0x780
        };
        struct
        {
            ULONGLONG* AllowedCpuSetsIndirect;                              //0x778
            ULONGLONG* DefaultCpuSetsIndirect;                              //0x780
        };
    };
};
/* Used in */
