...
struct _MM_SESSION_SPACE
{
    ...
    struct _MMSUPPORT Vm;                                                   //0xbf8
    struct _MMWSLE* Wsle;                                                   //0xc88
    struct _MI_SESSION_DRIVER_UNLOAD DriverUnload;                          //0xc90
    struct _POOL_DESCRIPTOR PagedPool;                                      //0xcc0
    struct _MMPTE PageDirectory;                                            //0x1e00
    struct _FAST_MUTEX SessionVaLock;                                       //0x1e08
    struct _RTL_BITMAP DynamicVaBitMap;                                     //0x1e40
    ULONG DynamicVaHint;                                                    //0x1e50
    struct _MI_SPECIAL_POOL SpecialPool;                                    //0x1e58
    struct _FAST_MUTEX SessionPteLock;                                      //0x1ea8
    LONG PoolBigEntriesInUse;                                               //0x1ee0
    ULONG PagedPoolPdeCount;                                                //0x1ee4
    ULONG SpecialPoolPdeCount;                                              //0x1ee8
    ULONG DynamicSessionPdeCount;                                           //0x1eec
    struct _MI_SYSTEM_PTE_TYPE SystemPteInfo;                               //0x1ef0
    VOID* PoolTrackTableExpansion;                                          //0x1f40
    ULONGLONG PoolTrackTableExpansionSize;                                  //0x1f48
    VOID* PoolTrackBigPages;                                                //0x1f50
    ULONGLONG PoolTrackBigPagesSize;                                        //0x1f58
    enum _IO_SESSION_STATE IoState;                                         //0x1f60
    ULONG IoStateSequence;                                                  //0x1f64
    struct _KEVENT IoNotificationEvent;                                     //0x1f68
};
