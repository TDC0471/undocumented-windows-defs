...
struct _EJOB
{
    ...
    union
    {
        ...
        struct _PS_WAKE_INFORMATION WakeInfo;                               //0x370
    };
    struct _JOBOBJECT_WAKE_FILTER WakeFilter;                               //0x3a8
    ULONG LowEdgeLatchFilter;                                               //0x3b0
    ULONG OwnedHighEdgeFilters;                                             //0x3b4
    struct _EJOB* NotificationLink;                                         //0x3b8
    ULONGLONG CurrentJobMemoryUsed;                                         //0x3c0
    struct _JOB_NOTIFICATION_INFORMATION* NotificationInfo;                 //0x3c8
    VOID* NotificationInfoQuotaReference;                                   //0x3d0
    struct _IO_MINI_COMPLETION_PACKET_USER* NotificationPacket;             //0x3d8
    struct _JOB_CPU_RATE_CONTROL* CpuRateControl;                           //0x3e0
    VOID* EffectiveSchedulingGroup;                                         //0x3e8
    ULONGLONG ReadyTime;                                                    //0x3f0
    struct _EX_PUSH_LOCK MemoryLimitsLock;                                  //0x3f8
    struct _LIST_ENTRY SiblingJobLinks;                                     //0x400
    struct _LIST_ENTRY ChildJobListHead;                                    //0x410
    struct _EJOB* ParentJob;                                                //0x420
    struct _EJOB* ParentSilo;                                               //0x428
    ...
    ULONGLONG TimerListLock;                                                //0x458
    struct _LIST_ENTRY TimerListHead;                                       //0x460
    struct _EPROCESS_VALUES Accounting;                                     //0x470
    ...
    ULONG EffectiveHighEdgeFilters;                                         //0x51c
    struct _PROCESS_ENERGY_VALUES* EnergyValues;                            //0x520
    volatile ULONGLONG SharedCommitCharge;                                  //0x528
    struct _EJOB* volatile WakeRoot;                                        //0x530
    ULONG DiskIoAttributionUserRefCount;                                    //0x538
    ULONG DiskIoAttributionRefCount;                                        //0x53c
    union
    {
        VOID* DiskIoAttributionContext;                                     //0x540
        struct _EJOB* DiskIoAttributionOwnerJob;                            //0x540
    };
    struct _PS_IO_CONTROL_ENTRY GlobalIoControl;                            //0x548
    volatile LONG VolumeIoControlLock;                                      //0x580
    struct _RTL_RB_TREE VolumeIoControlTree;                                //0x588
    struct _EX_PUSH_LOCK IoControlLock;                                     //0x598
    ULONGLONG SiloHardReferenceCount;                                       //0x5a0
    struct _WORK_QUEUE_ITEM RundownWorkItem;                                //0x5a8
};
/* Used in */
// _DEVICE_MAP
// _EJOB
