...
struct _POP_FX_DEVICE
{
    ...
    VOID* DriverContext;                                                    //0xa8
    struct _LIST_ENTRY AcpiLink;                                            //0xb0
    struct _UNICODE_STRING DeviceId;                                        //0xc0
    struct _IO_REMOVE_LOCK RemoveLock;                                      //0xd0
    struct _IO_REMOVE_LOCK AcpiRemoveLock;                                  //0xf0
    struct _POP_FX_WORK_ORDER WorkOrder;                                    //0x110
    ULONGLONG IdleLock;                                                     //0x148
    struct _KTIMER IdleTimer;                                               //0x150
    struct _KDPC IdleDpc;                                                   //0x190
    ULONGLONG IdleTimeout;                                                  //0x1d0
    ULONGLONG IdleStamp;                                                    //0x1d8
    struct _DEVICE_OBJECT* NextIrpDeviceObject[2];                          //0x1e0
    union _POWER_STATE NextIrpPowerState[2];                                //0x1f0
    VOID (*NextIrpCallerCompletion[2])(struct _DEVICE_OBJECT* arg1, UCHAR arg2, union _POWER_STATE arg3, VOID* arg4, struct _IO_STATUS_BLOCK* arg5); //0x1f8
    VOID* NextIrpCallerContext[2];                                          //0x208
    struct _KEVENT IrpCompleteEvent;                                        //0x218
    UCHAR (*PowerOnDumpDeviceCallback)(struct _PEP_CRASHDUMP_INFORMATION* arg1); //0x230
    struct _POP_FX_ACCOUNTING Accounting;                                   //0x238
    volatile ULONG Flags;                                                   //0x268
    ULONG ComponentCount;                                                   //0x26c
    struct _POP_FX_COMPONENT** Components;                                  //0x270
};
/* Used in */
// _DEVICE_NODE
// _POP_FX_COMPONENT
