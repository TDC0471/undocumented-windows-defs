...
struct _WMI_LOGGER_CONTEXT
{
    ...
    LONGLONG RelativeTimerDueTime;                                          //0x438
    struct _PERIODIC_CAPTURE_STATE_GUIDS PeriodicCaptureStateGuids;         //0x440
    struct _EX_TIMER* PeriodicCaptureStateTimer;                            //0x450
    enum _ETW_PERIODIC_TIMER_STATE PeriodicCaptureStateTimerState;          //0x458
    struct _ETW_SOFT_RESTART_CONTEXT* SoftRestartContext;                   //0x460
    struct _ETW_SILODRIVERSTATE* SiloState;                                 //0x468
    struct _WORK_QUEUE_ITEM CompressionWorkItem;                            //0x470
    LONG CompressionWorkItemState;                                          //0x490
    struct _EX_PUSH_LOCK CompressionLock;                                   //0x498
    struct _WMI_BUFFER_HEADER* CompressionTarget;                           //0x4a0
    VOID* CompressionWorkspace;                                             //0x4a8
    LONG CompressionOn;                                                     //0x4b0
    ULONG CompressionRatioGuess;                                            //0x4b4
    ULONG PartialBufferCompressionLevel;                                    //0x4b8
    enum ETW_COMPRESSION_RESUMPTION_MODE CompressionResumptionMode;         //0x4bc
    struct _SINGLE_LIST_ENTRY PlaceholderList;                              //0x4c0
    struct _KDPC CompressionDpc;                                            //0x4c8
    union _LARGE_INTEGER LastBufferSwitchTime;                              //0x508
    union _LARGE_INTEGER BufferWriteDuration;                               //0x510
    union _LARGE_INTEGER BufferCompressDuration;                            //0x518
    LONGLONG ReferenceQpcDelta;                                             //0x520
    struct _ETW_EVENT_CALLBACK_CONTEXT* CallbackContext;                    //0x528
    union _LARGE_INTEGER* LastDroppedTime;                                  //0x530
    union _LARGE_INTEGER* FlushingLastDroppedTime;                          //0x538
    LONGLONG FlushingSequenceNumber;                                        //0x540
    struct _ETW_PARTITION_CONTEXT PartitionContext;                         //0x548
    struct _MDL* BufferMdl;                                                 //0x550
};
/* Used in */
