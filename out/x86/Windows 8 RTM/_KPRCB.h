...
struct _KPRCB
{
    ...
    UCHAR PendingTick;                                                      //0x3d1
    UCHAR PrcbPad1[70];                                                     //0x3d2
    ...
    UCHAR PrcbPad50[48];                                                    //0x21a8
    ULONG InterruptLastCount;                                               //0x21d8
    ULONG InterruptRate;                                                    //0x21dc
    struct _KDPC_DATA DpcData[2];                                           //0x21e0
    VOID* DpcStack;                                                         //0x2208
    LONG MaximumDpcQueueDepth;                                              //0x220c
    ULONG DpcRequestRate;                                                   //0x2210
    ULONG MinimumDpcRate;                                                   //0x2214
    ULONG DpcLastCount;                                                     //0x2218
    ULONG PrcbLock;                                                         //0x221c
    struct _KGATE DpcGate;                                                  //0x2220
    UCHAR ThreadDpcEnable;                                                  //0x2230
    volatile UCHAR QuantumEnd;                                              //0x2231
    volatile UCHAR DpcRoutineActive;                                        //0x2232
    volatile UCHAR IdleSchedule;                                            //0x2233
    union
    {
        volatile LONG DpcRequestSummary;                                    //0x2234
        SHORT DpcRequestSlot[2];                                            //0x2234
        struct
        {
            SHORT NormalDpcState;                                           //0x2234
            SHORT ThreadDpcState;                                           //0x2236
        };
        struct
        {
            ULONG DpcNormalProcessingActive:1;                              //0x2234
            ULONG DpcNormalProcessingRequested:1;                           //0x2234
            ULONG DpcNormalThreadSignal:1;                                  //0x2234
            ULONG DpcNormalTimerExpiration:1;                               //0x2234
            ULONG DpcNormalDpcPresent:1;                                    //0x2234
            ULONG DpcNormalLocalInterrupt:1;                                //0x2234
            ULONG DpcNormalSpare:10;                                        //0x2234
            ULONG DpcThreadActive:1;                                        //0x2234
            ULONG DpcThreadRequested:1;                                     //0x2234
            ULONG DpcThreadSpare:14;                                        //0x2234
        };
    };
    ULONG LastTimerHand;                                                    //0x2238
    ULONG LastTick;                                                         //0x223c
    ULONG PeriodicCount;                                                    //0x2240
    ULONG PeriodicBias;                                                     //0x2244
    ULONG ClockInterrupts;                                                  //0x2248
    ULONG ReadyScanTick;                                                    //0x224c
    UCHAR BalanceState;                                                     //0x2250
    UCHAR GroupSchedulingOverQuota;                                         //0x2251
    UCHAR PrcbPad41[10];                                                    //0x2252
    ...
    struct _LIST_ENTRY WaitListHead;                                        //0x3ae0
    ULONG WaitLock;                                                         //0x3ae8
    ULONG ReadySummary;                                                     //0x3aec
    ULONG ReadyQueueWeight;                                                 //0x3af0
    struct _KPRCB* BuddyPrcb;                                               //0x3af4
    ULONGLONG StartCycles;                                                  //0x3af8
    ULONGLONG GenerationTarget;                                             //0x3b00
    volatile ULONGLONG CycleTime;                                           //0x3b08
    volatile ULONG HighCycleTime;                                           //0x3b10
    ULONG ScbOffset;                                                        //0x3b14
    ULONGLONG AffinitizedCycles;                                            //0x3b18
    ...
    ULONG Stride;                                                           //0x3ca8
    ULONG PrcbPad90;                                                        //0x3cac
    struct _PROCESSOR_POWER_STATE PowerState;                               //0x3cb0
    ULONG PrcbPad91[1];                                                     //0x3e30
    struct _KDPC DpcWatchdogDpc;                                            //0x3e34
    struct _KTIMER DpcWatchdogTimer;                                        //0x3e58
    union _SLIST_HEADER HypercallPageList;                                  //0x3e80
    VOID* HypercallPageVirtual;                                             //0x3e88
    VOID* VirtualApicAssist;                                                //0x3e8c
    ULONGLONG* StatisticsPage;                                              //0x3e90
    struct _CACHE_DESCRIPTOR Cache[5];                                      //0x3e94
    ULONG CacheCount;                                                       //0x3ed0
    struct _KAFFINITY_EX PackageProcessorSet;                               //0x3ed4
    ULONG CacheProcessorMask[5];                                            //0x3ee0
    ULONG ScanSiblingMask;                                                  //0x3ef4
    ULONG CoreProcessorSet;                                                 //0x3ef8
    ULONG ScanSiblingIndex;                                                 //0x3efc
    ULONG LLCLevel;                                                         //0x3f00
    VOID* WheaInfo;                                                         //0x3f04
    VOID* EtwSupport;                                                       //0x3f08
    union _SLIST_HEADER InterruptObjectPool;                                //0x3f10
    ULONG PrcbPad92[8];                                                     //0x3f18
    struct _PROCESSOR_PROFILE_CONTROL_AREA* ProcessorProfileControlArea;    //0x3f38
    VOID* ProfileEventIndexAddress;                                         //0x3f3c
    struct _KDPC TimerExpirationDpc;                                        //0x3f40
    struct _SYNCH_COUNTERS SynchCounters;                                   //0x3f60
    struct _FILESYSTEM_DISK_COUNTERS FsCounters;                            //0x4018
    struct _CONTEXT* Context;                                               //0x4028
    ULONG ContextFlagsInit;                                                 //0x402c
    struct _XSAVE_AREA* ExtendedState;                                      //0x4030
    struct _KENTROPY_TIMING_STATE EntropyTimingState;                       //0x4034
};
/* Used in */
// _KPCR
// _KPRCB
