...
struct _MM_SESSION_SPACE
{
    ...
    struct _MMSUPPORT Vm;                                                   //0xcec
    struct _MMWSLE* Wsle;                                                   //0xd5c
    struct _MI_SESSION_DRIVER_UNLOAD DriverUnload;                          //0xd60
    struct _POOL_DESCRIPTOR PagedPool;                                      //0xd80
    struct _MMPTE* PageTables;                                              //0x1ec0
    struct _MI_SPECIAL_POOL SpecialPool;                                    //0x1ec8
    struct _FAST_MUTEX SessionPteLock;                                      //0x1f10
    LONG PoolBigEntriesInUse;                                               //0x1f30
    ULONG PagedPoolPdeCount;                                                //0x1f34
    ULONG SpecialPoolPdeCount;                                              //0x1f38
    ULONG DynamicSessionPdeCount;                                           //0x1f3c
    struct _MI_SYSTEM_PTE_TYPE SystemPteInfo;                               //0x1f40
    VOID* PoolTrackTableExpansion;                                          //0x1f74
    ULONG PoolTrackTableExpansionSize;                                      //0x1f78
    VOID* PoolTrackBigPages;                                                //0x1f7c
    ULONG PoolTrackBigPagesSize;                                            //0x1f80
    enum _IO_SESSION_STATE IoState;                                         //0x1f84
    ULONG IoStateSequence;                                                  //0x1f88
    struct _KEVENT IoNotificationEvent;                                     //0x1f8c
    struct _RTL_BITMAP SessionPoolPdes;                                     //0x1f9c
};
