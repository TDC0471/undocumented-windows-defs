...
struct _POP_IRP_DATA
{
    ...
    union
    {
        struct
        {
            VOID (*CallerCompletion)(struct _DEVICE_OBJECT* arg1, UCHAR arg2, union _POWER_STATE arg3, VOID* arg4, struct _IO_STATUS_BLOCK* arg5); //0x80
            VOID* CallerContext;                                            //0x84
            struct _DEVICE_OBJECT* CallerDevice;                            //0x88
            UCHAR SystemWake;                                               //0x8c
        } Device;                                                           //0x80
        struct
        {
            struct _PO_DEVICE_NOTIFY* NotifyDevice;                         //0x80
            UCHAR FxDeviceActivated;                                        //0x84
        } System;                                                           //0x80
    };
};
/* Used in */
