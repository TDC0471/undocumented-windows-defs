...
struct _WMI_LOGGER_CONTEXT
{
    ...
    LONGLONG RelativeTimerDueTime;                                          //0x280
    struct _PERIODIC_CAPTURE_STATE_GUIDS PeriodicCaptureStateGuids;         //0x288
    struct _EX_TIMER* PeriodicCaptureStateTimer;                            //0x290
    enum _ETW_PERIODIC_TIMER_STATE PeriodicCaptureStateTimerState;          //0x294
    struct _ETW_SOFT_RESTART_CONTEXT* SoftRestartContext;                   //0x298
    struct _ETW_SILODRIVERSTATE* SiloState;                                 //0x29c
    struct _WORK_QUEUE_ITEM CompressionWorkItem;                            //0x2a0
    LONG CompressionWorkItemState;                                          //0x2b0
    struct _EX_PUSH_LOCK CompressionLock;                                   //0x2b4
    struct _WMI_BUFFER_HEADER* CompressionTarget;                           //0x2b8
    VOID* CompressionWorkspace;                                             //0x2bc
    LONG CompressionOn;                                                     //0x2c0
    ULONG CompressionRatioGuess;                                            //0x2c4
    ULONG PartialBufferCompressionLevel;                                    //0x2c8
    enum ETW_COMPRESSION_RESUMPTION_MODE CompressionResumptionMode;         //0x2cc
    struct _SINGLE_LIST_ENTRY PlaceholderList;                              //0x2d0
    struct _KDPC CompressionDpc;                                            //0x2d4
    union _LARGE_INTEGER LastBufferSwitchTime;                              //0x2f8
    union _LARGE_INTEGER BufferWriteDuration;                               //0x300
    union _LARGE_INTEGER BufferCompressDuration;                            //0x308
};
/* Used in */
